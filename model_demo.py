...
class DDoSDetector:
    ...
    def setup_preprocessing(self):
        """Setup preprocessing pipeline using realistic dataset"""
        try:
            # Prefer combined realistic dataset, fallback to train part
            realistic_paths = [
                "data/optimized/realistic_balanced_dataset.csv",
                "data/optimized/realistic_train.csv"
            ]
            sample_data = None
            for p in realistic_paths:
                if os.path.exists(p):
                    sample_data = pd.read_csv(p)
                    logger.info(f"Using preprocessing sample data: {p}")
                    break
            if sample_data is None:
                raise FileNotFoundError(
                    "No realistic dataset found for preprocessing.")

            feature_columns = [
                col for col in sample_data.columns if col != 'Binary_Label']
            ...
            logger.info(
                f"✅ Preprocessing setup complete. Features: {len(self.feature_names)}")
    ...
    def preprocess_data(self, data: np.ndarray) -> np.ndarray:
        ...
        data_reshaped = data_scaled.reshape(
            data_scaled.shape[0], data_scaled.shape[1], 1)
    ...
    def batch_predict_from_file(self, file_path: str, sample_size: int = 1000) -> dict:
        ...
        if len(df) > sample_size:
            ...
            logger.info(
                f"📊 Sampled {sample_size} records from {len(df)} total")
        ...
        if actual_labels is not None:
            predicted_labels = np.array(
                [1 if r["is_attack"] else 0 for r in results["predictions"]])
...
def create_sample_traffic():
    ...
    try:
        ...
        for p in realistic_paths:
            if os.path.exists(p):
                df = pd.read_csv(p)
                logger.info(f"Using sample source: {p}")
                break
        if df is None:
            raise FileNotFoundError(
                "No realistic dataset found for sample traffic.")

        # Sample some benign and attack traffic
        benign_samples = df[df['Binary_Label']
                            == 0].sample(n=5, random_state=42)
        attack_samples = df[df['Binary_Label']
                            == 1].sample(n=5, random_state=42)

        sample_data = pd.concat(
            [benign_samples, attack_samples]).reset_index(drop=True)
        feature_columns = [
            col for col in sample_data.columns if col != 'Binary_Label']
        ...
        logger.error(f"❌ Failed to load realistic sample data: {str(e)}")
        raise
...
def demo_real_time_detection():
    ...
    # Initialize detector
    detector = DDoSDetector("results/best_enhanced_model.keras")
    ...
    for i, prediction in enumerate(results["predictions"]):
        ...
        status = "✅ CORRECT" if (prediction["is_attack"] and actual_labels[i] == 1) or (
            not prediction["is_attack"] and actual_labels[i] == 0) else "❌ INCORRECT"

        logger.info(
            f"Sample {i+1:2d}: {prediction['prediction']:12s} | Confidence: {prediction['confidence']:.3f} | Risk: {prediction['risk_level']:8s} | Actual: {actual:6s} | {status}")
    ...
    logger.info(
        f"Processing Speed:  {summary['samples_per_second']:.1f} samples/second")
...
def benchmark_model_performance():
    ...
    for i, test_file in enumerate(test_files):
        if os.path.exists(test_file):
            ...
            results = detector.batch_predict_from_file(
                test_file, sample_size=500)
            ...
            if "validation" in results:
                ...
                logger.info(
                    f"True Positives: {results['validation']['true_positives']}")
                logger.info(
                    f"False Positives: {results['validation']['false_positives']}")

            total_samples += results["summary"]["total_samples"]
            inference_time = datetime.fromisoformat(
...
if __name__ == "__main__":
    try:
        ...
        if not os.path.exists(model_path):
            ...
            logger.error(
                "Please train the model first using: python train_enhanced.py")
            sys.exit(1)
