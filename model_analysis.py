...
class ModelAnalyzer:
    ...
    def load_test_data(self):
        ...
        try:
            ...
            if 'Binary_Label' in df.columns:
                ...
                feature_columns = [
                    col for col in df.columns if col != 'Binary_Label']
                ...
                if df['Label'].dtype == 'object':
                    ...
                    label_col = 'Label'
                feature_columns = [
                    col for col in df.columns if col not in ['Label', 'Binary_Label']]
            else:
                # Assume last column is label
                label_col = df.columns[-1]
                if df[label_col].dtype == 'object':
                    df['Binary_Label'] = (
                        df[label_col] != 'BENIGN').astype(int)
                    label_col = 'Binary_Label'
    ...
    def analyze_confusion_matrix(self):
        ...
        logger.info(
            f"  False Positive Rate: {error_analysis['false_positive_rate']:.4f}")
        logger.info(
            f"  False Negative Rate: {error_analysis['false_negative_rate']:.4f}")
    ...
    def analyze_confidence_distribution(self):
        ...
        logger.info(
            f"  Benign Traffic - Mean Confidence: {confidence_analysis['benign_confidence']['mean']:.4f}")
        logger.info(
            f"  Attack Traffic - Mean Confidence: {confidence_analysis['attack_confidence']['mean']:.4f}")
    ...
    def create_advanced_visualizations(self):
        ...
        fig.suptitle('Advanced DDoS Detection Model Analysis',
        ...
        precision_curve, recall_curve, _ = precision_recall_curve(
            self.y_test, self.y_pred_proba)
        pr_auc = average_precision_score(self.y_test, self.y_pred_proba)
        axes[0, 2].plot(recall_curve, precision_curve, color='darkgreen',
        ...
        benign_confidence = self.y_pred_proba[self.y_test == 0]
        attack_confidence = self.y_pred_proba[self.y_test == 1]
        axes[1, 0].hist(benign_confidence, bins=50, alpha=0.7,
                        label='Benign', color='blue', density=True)
        axes[1, 0].hist(attack_confidence, bins=50, alpha=0.7,
        ...
        axes[1, 1].axvline(x=0.5, color='black', linestyle='--',
                           alpha=0.7, label='Default Threshold')
        ...
        plt.tight_layout()
        plt.savefig('results/advanced_model_analysis.png',
                    dpi=300, bbox_inches='tight')
        logger.info(
            "📊 Advanced visualizations saved to: results/advanced_model_analysis.png")
    ...
    def generate_comprehensive_report(self):
        ...
        logger.info(
            "💾 Comprehensive report saved to: results/comprehensive_model_analysis.json")
    ...
    def generate_recommendations(self, performance_metrics: dict, error_analysis: dict) -> list:
        ...
        if performance_metrics["accuracy"] < 0.90:
            recommendations.append(
                "Consider increasing model complexity or training duration for better accuracy")

        if performance_metrics["precision"] < 0.90:
            recommendations.append(
                "High false positive rate - consider adjusting classification threshold or improving feature engineering")

        if performance_metrics["recall"] < 0.85:
            recommendations.append(
                "High false negative rate - critical for security, consider ensemble methods or more attack samples")

        # Error analysis recommendations
        if error_analysis["false_positive_rate"] > 0.10:
            recommendations.append(
                "Consider reducing false positives to minimize disruption to legitimate traffic")

        if error_analysis["false_negative_rate"] > 0.15:
            recommendations.append(
                "High miss rate for attacks - consider additional training data or feature engineering")

        # General recommendations
        recommendations.append(
            "Regular model retraining recommended as attack patterns evolve")
        recommendations.append(
            "Consider implementing ensemble methods for improved robustness")
        recommendations.append(
            "Monitor model performance in production and retrain when performance degrades")
...
def main():
    ...
    model_path = "results/best_enhanced_model.keras"
    data_path = "data/optimized/realistic_balanced_dataset.csv"
    ...
    try:
        ...
        logger.info(
            f"Model Parameters: {report['model_summary']['total_parameters']:,}")
        logger.info(
            f"Accuracy: {report['performance_metrics']['accuracy']:.4f}")
        logger.info(
            f"F1-Score: {report['performance_metrics']['f1_score']:.4f}")
